...
\setmainfont[
    Ligatures=TeX,
    StylisticSet=01, % single-story a and other Futura-like fun.
...
\newcommand{\cpp}[1]{C\kern-0.1ex\raisebox{0.15ex}{++}{#1}}
\newcommand{\clang}[1]{C{#1}}
...
sharing the CPU in short slices of time.
...
Modern CPU designs handle incoming instructions in
...
\captionof{figure}{A traditional five-stage CPU pipeline
...
RAM hasn't been able to keep up,
...
hierarchical caches directly on the CPU die.
...
Fortunately, the 2011 ISO standards of both languages introduced
tools for the task.
...
There are a few common \introduce{read-modify-write} (RMW) operations.
...
The simplest atomic RMW operation is an \introduce{exchange}:
...
To do so, we'll have the UI thread zero the counter each time it
is read.
Even if these reads and writes are atomic,
we could still run into the following race condition:
\begin{enumerate}
\item The UI thread reads the counter.
\item Before the UI thread has the chance to zero it,
    the worker thread increments it again.
\item The UI thread now zeroes the counter, and the previous increment
    is lost.
\end{enumerate}
If the UI thread exchanges the current
...
\item Before that thread can store the value back,
    the UI thread zeroes the counter.
...
Finally, we have \introduce{compare-and-swap} (CAS),
...
In \clang{} and \cpp{}, CAS resembles the following,
...
Atomic loads, stores, and RMW operations are the building
...
In the second, subsequent sensor inputs could be missed if the ISR
...
algorithm even though it doesn't use any OS-provided syscalls to
put the blocked thread to sleep.\punckern\footnote{Putting a blocked thread
to sleep is often an optimization,
since the operating system's scheduler can run other threads on the CPU
...
is blocked for less than the spin length, but avoids wasting CPU
...
play to the specifics of your CPU hardware.
...
Other architectures like ARM are more \introduce{weakly-ordered},
so one shouldn't assume that loads and stores are executed in
program order unless the CPU is given special instructions---called
...
similar models.}
Let's examine ARM, since it's straightforward and widely-used.
...
Like many other
RISC\footnote{\introduce{Reduced instruction set computer},
in contrast to a \introduce{complex instruction set computer} (CISC)
architecture like x64.}
architectures, ARM lacks dedicated RMW instructions.
Since the processor can context switch to another thread
between any two instructions,
we can't implement RMW ops with normal loads and stores.
Instead, we need
\introduce{load-link} and \introduce{store-conditional} (LL/SC).
...
\end{lstlisting}
\end{colfigure}
We LL the current value, add one, and immediately try to store it back
with a SC. If that fails, another thread may have
written a new value to \texttt{foo} since our LL,
...
Unlike the \monobox{\_strong} version, a weak CAS
is allowed to fail spuriously, just like the underlying LL/SC mechanism.
...
We've also seen how weakly-ordered architectures like ARM
...
and the various flavors of RMW---are sequentially consistent.
But this is only one of several orderings that we can give them.
We'll examine each of them in turn, but a full list,
along with the enumerations that the \clang{} and \cpp{} API uses,
is:
\begin{itemize}
...
\texttt{exchange()} becomes \texttt{exchange\_explicit()}, a CAS
...
orderings: one for when the CAS succeeds, and one for when it fails:
\begin{colfigure}
\begin{minted}[fontsize=\codesize]{cpp}
...
On ARM and other weakly-ordered architectures, this allows us to drop
...
thread increments a counter which is read by a UI thread to show
progress.
...
\texttt{memory\_order\_acq\_rel} is used with atomic RMW operations
...
the ISO committee's concurrency study group, SG1,
came away with different understandings.
See
\href{http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2014/n4036.pdf}{N4036}
...
Those familiar with the platform may have noticed that all ARM assembly
...
exploring, ARMv8 processors offer dedicated load-acquire
...
Recall that memory is transferred between main RAM and the CPU
...
two orders of magnitude than an atomic RMW operation.
...
into instructions for the devices connected to the CPU.
...
\item The compiler will not elide what it otherwise sees as ``unnecessary''
    ...
    But, if \texttt{t} points to some MMIO register, it's not
...
\href{http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2015/n4374.html}{N4374}
and the kernel's
...
\begin{itemize}
\item Why compilers and CPU hardware reorder loads and stores.
...
\item How atomic operations can be implemented on weakly-ordered hardware,
    and what implications this can have for a language-level API.
...
(SP/SC) queue or \introduce{read-copy-update}
(RCU).\punckern\footnote{See the Linux Weekly News article,
...
CPU architecture specifics, and more.

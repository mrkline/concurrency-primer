...
Seasoned programmers are quite familiar with concurrency
building blocks like mutexes, semaphores, and condition variables.
But how do those things actually work?
...
or when blocking is unacceptable due to hard time constraints?
And since your system transforms your code into things you didn't write,
running in orders you didn't ask for, how do multithreaded programs work at all?
...
For these interactions to work, we must be able to reason about
...
Our system must guarantee that other threads observe \textit{A's} write to the
flag only \emph{after A's} write to the integer.
(If another thread can observe a change to the flag before it observes a change
to the integer, it could ``see'' an incorrect value,
keeping us from reliably exchanging information.)
...
it runs on.
So long as the resulting instructions will run to the same effect
...
to suspend forward progress, or \introduce{stall,} until the needed result is ready.}
...
Modern desktop and server \textsc{cpu} designs handle incoming instructions in
...
where each core takes turns performing reads and writes to a single shared memory.
\begin{colfigure}
\centering
...
even if they use different caches,
is very challenging.
...
Let's return to our simple example from before.
A naïve programmer, blissfully unaware of the challenges we've discussed,
might write:
\begin{colfigure}
...
\end{minted}
\end{colfigure}
For this to work as-desired, all we need to do is use an
\introduce{atomic type} for the flag:
\begin{minted}[fontsize=\codesize]{cpp}
...
In our example, \monobox{v = 42}\, is now guaranteed to happen before
\monobox{v\_ready = true}\, in thread \textit{A},
just as \monobox{my\_v = v}\, must occur after reading \monobox{v\_ready}\,
in thread~\textit{B.}

Formally, these types ensure a \textit{single total modification order}
such that,
...
Our focus so far on ordering has ignored the other vital element of lockless
...
let's see what might happen if they did not.
...
From this example we can see why we need atomic reads and writes
...
the architecture's native word size.
...
since we're usually using atomic operations to avoid locks in the first place.}
...
This might be necessary is for some architectures that
doesn't guarantee atomicity for unaligned reads and writes.}
...
\item The \textsc{ui} thread now zeroes the counter, and the previous increment
    is lost.
\end{enumerate}
If the \textsc{ui} thread atomically exchanges the current
value of the counter with zero, the race disappears.
...
wait until they unlock it by clearing the flag.
...
we need the worker thread's additions to be atomic too,
...
\item The worker now performs its store.
    It is now as if the counter was never cleared.
\end{enumerate}
...
\begin{samepage}
\noindent You might be a bit perplexed by the \texttt{\_strong} suffix.
...
\end{minted}
\end{colfigure}
Say we only want to set \texttt{ts} to \texttt{Cancelled} if it's
currently \texttt{Running}, but do nothing if it's already \texttt{Idle}.
This is trivial with a \textsc{cas}:
\begin{colfigure}
\begin{minted}[fontsize=\codesize]{cpp}
bool cancel()
{
...
Some of these examples might seem a bit contrived,
but atomic \textsc{rmw} operations are the building blocks for all sorts of
lock-free data structures and algorithms,
along with the blocking concurrency primitives we know and love.
To see how they can be used with syscalls to create
mutexes and condition variables in Linux, for example,
read \textit{Futexes Are Tricky} by Ulrich Drepper.
...
Well-written locks can be very efficient if you need to share data
between threads.
...
so a lock-free approach is your only recourse.

Of course, there are many situations where blocking
...
There are no easy answers here---how well a given synchronization method will
...
And as always, consider the complexity versus performance
...
x64, for example, has a relatively \introduce{strongly-ordered} model---while
the \textsc{cpu} will aggressively rearrange your program,
it can mostly be trusted to preserve some system-wide order of
loads and stores.
Other architectures like \textsc{arm} have a \introduce{weakly-ordered}
model, where one should make very few assumptions that loads and stores are in
...
both to gain a better understanding of what's actually happening in hardware,
and to understand why the \clang{} and \cpp{} models were designed as they
...
Many \textsc{risc}\footnote{\introduce{Reduced instruction set computer},
in contrast to a \introduce{complex instruction set computer} \textsc{(cisc)}
architecture like x64.}architectures lack single instructions to perform
...
We need something special: load-link and store-conditional
\textsc{(ll/sc)}.
...
Consider an atomic fetch and add:
\begin{colfigure}
\begin{minted}[fontsize=\codesize]{cpp}
void incFoo() { ++foo; }
\end{minted}
...
byte-addressable level can be very expensive in terms of \textsc{cpu} hardware.
To reduce this cost, many processors monitor them at some coarser
granularity, such as a cache line.
...
and an outer one which repeatedly loads and multiplies values of \texttt{foo} until no
other thread has modified it.
When we use \monobox{compare\_exchange\_weak} instead,
...
to ensure that other memory accesses aren't rearranged in ways that break our
programs.
We've also seen how weakly-ordered architectures like \textsc{arm}
use a pair of memory barriers to ensure this guarantee.
As you might expect, these barriers can have a non-trivial impact on performance
since they inhibit optimizations that your compiler and hardware would otherwise
make.
...
we read and write to things that aren't
...
But this is actually only one of several orderings that we can give them.
...
\end{itemize}
To specify an order, you provide it as an optional argument that we've slyly
...
orders: one for when the \textsc{cas} succeeds, and one for when it fails:
\begin{colfigure}
\begin{minted}[fontsize=\codesize]{cpp}
...
We've already seen acquire and release in action with the lock example in
\secref{lock-example}.
...
letting them move $after \to before$.
\begin{colfigure}
\centering
\includegraphics[keepaspectratio,width=0.9\linewidth]{acquire-release}
\captionof{figure}{An acquire and a release operation work together as a pair.}
...
We can use this pairing for our very first example from
...
Relaxed atomic operations are needed when a variable will be shared between threads,
but \emph{no ordering} is required.
\begin{colfigure}
\centering
...
\label{oppenheimer}
\end{colfigure}
This might seem useless at first glance, but has several common
applications.
Recall our examples from \secref{atomicity} and \secref{rmw} where some worker
thread increments a counter which is then read by a \textsc{ui} thread to show
progress.
That counter could be incremented with
\texttt{atomic\_fetch\_add()} using \texttt{memory\_order\_relaxed}.
All we need is atomicity---nothing is ordered or synchronized
by the counter.
...
worker should exit and doesn't ``announce'' any sort of new data that will be
read by the worker.
...
A typical example is thread-safe reference counting, like in \cpp{}'s
\mintinline{cpp}{shared_ptr}:
\begin{colfigure}
\begin{minted}[fontsize=\codesize]{cpp}
atomic_int refCount;
...
\begin{enumerate}
\item All reads and writes to the object being reference-counted happen
\emph{before} the count reaches zero.
\item Deletion occurs \emph{after} the reference count drops to
    zero.\punckern\footnote{This can actually be optimized even further by
...
Consider some data that is rarely changed, but often read by multiple
threads.
Perhaps it is an in-kernel pointer to information about the peripherals
plugged into the machine.
This data will change \emph{very} infrequently,
so it makes sense to optimize reads as much as possible.
Given what we know so far, the most efficient approach we have is:
\begin{colfigure}
\begin{minted}[fontsize=\codesize]{cpp}
// A writer:
...
it would be really nice if we could avoid a memory barrier when reading
...
most platforms---even weakly-ordered ones---cannot make the initial
load (\mintinline{cpp}{p = peripherals}) take place after its use
...
DEC Alpha.}
So long as we can convince the compiler not to make any similar speculations,
we're in the clear.
...
Non-sequentially consistent orderings can be quite tricky,
...
resorting to memory barriers---even sequentially consistent operations.
...
and the optimizer must be inhibited with \mintinline{cpp}{volatile} casts
or incantations like
...
macros for this very purpose.\punckern\footnote{See
...
but we've hopefully provided an understanding of:
\begin{itemize}
...
\item How we can \emph{carefully} optimize our lockless code using alternative
    memory orderings.
\item How the compiler might optimize our atomic operations even further,
    and what we can do to prevent certain undesirable optimizations.
\end{itemize}
To learn more, see the additional resources below,
or examine some lock-free data structures and algorithms,

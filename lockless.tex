...
To avoid them, we need atomic reads and writes to synchronize threads.
...
\end{minted}
\end{colfigure}
If we only want to set \texttt{ts} to \texttt{Cancelled} if it's
...
so one should make very few assumptions that loads and stores are in
